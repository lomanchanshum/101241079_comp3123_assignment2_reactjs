...
const EmpListing = () => {
    ...
    const Removefunction = (id) => {
        if (window.confirm('Do you want to remove?')) {
            fetch("http://localhost:5000/api/emp/employees/" + id, {
                method: "DELETE"
            }).then((res) => {
    ...
    useEffect(() => {
        fetch("http://localhost:5000/api/emp/employees/").then((res) => {
            return res.json();
        }).then((resp) => {
            empdatachange(resp);
            // console.log(resp)
                                ...
                                empdata.map(item => (
                                    <tr key={item._id}>
                                        ...
                                        <td><a onClick={() => { LoadEdit(item._id) }} className="btn btn-success">Edit</a>
                                            ...
                                            <a onClick={() => { LoadDetail(item._id) }} className="btn btn-success">Details</a>
                                        </td>
                                    </tr>
                                ))
                            }
